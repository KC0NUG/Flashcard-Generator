...
inquirer.prompt([{
    ...
    if (answer.command === 'add-flashcard') {
        addCard();
    } else if (answer.command === 'play-game') {
        showCards();
    }
});
...
var menu = function() {
    ...
    // once user input is received
        ...
        if (answer.menuCommand === 'create-new-card') {
            addCard();
        } else if (answer.menuCommand === 'play-game') {
            showCards();
        } else if (answer.menuCommand === 'exit-game') {
            return;
        }
    });
};
...
var showQuestion = function(array, index) {
    ...
    inquirer.prompt([{
        ...
        if (answer.response === correctReponse) {
            ...
            console.log('Incorrect!  The correct response is: ' + correctReponse );
            // go to next question
            if (index < array.length - 1) {
              showQuestion(array, index + 1);
            }
        ...
        if (answer.cardType === 'basic-flashcard') {
            ...
            inquirer.prompt([{
                ...
                if (text.includes(cloze)) {
                    ...
                    console.log('The cloze portion you provided was not found . Please try again.');
                    addCard();
                }
            });
        }
